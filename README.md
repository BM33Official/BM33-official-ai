...
## Control Center (ระบบบรอดแคสต์) — `/admin`

หน้าเว็บควบคุมการยิงประกาศแบบ human-in-the-loop (ต้องล็อกอินด้วย `ADMIN_PANEL_PASSWORD`)

- **ลงทะเบียนสมาชิก**: เพื่อนแอด OA → บอทถามชื่อ + 3 ตัวท้ายรหัส นศ. → จับคู่กับ `BC_roster` → ยืนยัน
- **ติดตามฟอร์ม**: วางลิงก์ response sheet + เลือกคอลัมน์รหัส นศ. → ระบบรู้ว่าใครยังไม่ทำ
- **บรอดแคสต์**: เขียน → พรีวิว → อนุมัติ → ส่งเฉพาะคนที่ยังไม่ทำ (การ์ด/ข้อความ, ตั้งเวลา, เตือนซ้ำ)
- **ปุ่ม "ทำแล้ว"**: สมาชิกกด → AI อ่านชีตยืนยันเอง หรือเข้าคิวให้แอดมินยืนยันใน "กล่องรอตรวจ"
- **โหมดทดสอบ**: ส่งหาแอดมินเท่านั้น (ไม่กินโควตาจริง) — เปิดไว้จนกว่าจะพร้อม

โครงสร้าง: `lib/bc/**` (onboarding, roster, members, forms, status, broadcast, cron, auth) + `app/admin/**` (UI) + `app/api/broadcast-cron` (ยิงตามเวลา ผ่าน `.github/workflows/broadcast.yml`).
เก็บข้อมูลในแท็บ `BC_roster / BC_members / BC_forms / BC_status / BC_broadcasts / BC_send_log` (สร้างอัตโนมัติ).

ตั้งค่าเพิ่ม: `ADMIN_PANEL_PASSWORD`, `BROADCAST_CRON_SECRET` (Vercel) + `BROADCAST_URL`, `BROADCAST_CRON_SECRET` (GitHub Secrets).
**ต้องกรอกทะเบียนรุ่นใน `BC_roster`** (student_id | full_name | nickname | notes) ก่อน onboarding ถึงจะจับคู่ได้.
